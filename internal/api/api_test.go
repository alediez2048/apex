...
func TestSettlement_Returns501(t *testing.T) {
	cfg := testCfg()
	handler := api.SettlementHandler(cfg)
	...
	if m["code"] != "NOT_IMPLEMENTED" {
		t.Fatalf("want NOT_IMPLEMENTED, got %v", m["code"])
	}
}

// --- Returns stubs ---

func TestReturns_Returns501(t *testing.T) {
	cfg := testCfg()
	handler := api.ReturnsHandler(cfg)

	req := httptest.NewRequest(http.MethodPost, "/api/v1/returns", nil)
	req.Header.Set("Authorization", "Bearer tok_alice_001")
	w := httptest.NewRecorder()
	handler(w, req)
